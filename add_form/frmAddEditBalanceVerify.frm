...
Begin VB.Form frmAddEditBalanceVerify 
   ClientHeight    =   8490
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   11880
   Icon            =   "frmAddEditBalanceVerify.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   Moveable        =   0   'False
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin MSComctlLib.TabStrip TabStrip1 
         ...
         EndProperty
      End
      Begin PorkShop.uctlTextBox txtNo 
         Height          =   435
         Left            =   1860
         TabIndex        =   0
         Top             =   840
      ...
      Begin GridEX20.GridEX GridEX1 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "frmAddEditBalanceVerify.frx":27A2
         Column(2)       =   "frmAddEditBalanceVerify.frx":286A
         FormatStylesCount=   5
         FormatStyle(1)  =   "frmAddEditBalanceVerify.frx":290E
         FormatStyle(2)  =   "frmAddEditBalanceVerify.frx":2A6A
         FormatStyle(3)  =   "frmAddEditBalanceVerify.frx":2B1A
         FormatStyle(4)  =   "frmAddEditBalanceVerify.frx":2BCE
         FormatStyle(5)  =   "frmAddEditBalanceVerify.frx":2CA6
         ImageCount      =   0
         PrinterProperties=   "frmAddEditBalanceVerify.frx":2D5E
      End
      Begin Threed.SSPanel pnlHeader 
         ...
         PictureBackgroundStyle=   2
      End
      Begin PorkShop.uctlTextBox txtDesc 
         Height          =   435
         Left            =   1860
         TabIndex        =   1
         Top             =   1320
         ...
         _ExtentY        =   767
      End
      Begin PorkShop.uctlDate uctlDate 
         Height          =   405
         Left            =   6360
         TabIndex        =   13
         Top             =   840
         ...
         _ExtentY        =   714
      End
      Begin Threed.SSCommand cmdMakeAdjust 
         Height          =   525
         Left            =   10200
         TabIndex        =   15
         Top             =   1260
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmAddEditBalanceVerify.frx":2F36
         ButtonStyle     =   3
      End
      ...
      Begin Threed.SSCommand cmdOK 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmAddEditBalanceVerify.frx":3250
         ButtonStyle     =   3
      End
      ...
      Begin Threed.SSCommand cmdAdd 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmAddEditBalanceVerify.frx":356A
         ButtonStyle     =   3
      End
   ...
   End
End
Attribute VB_Name = "frmAddEditBalanceVerify"
...
Private Sub cmdMakeAdjust_Click()
...
Dim InventoryBals2 As Collection
   ...
   If Not VerifyLockDate(uctlDate.ShowDate, uctlDate.ShowDate) Then
   ...
   If Not VerifyLockInventoryDate(uctlDate.ShowDate, uctlDate.ShowDate) Then
      glbErrorLog.LocalErrorMsg = MapText("ไม่สามารถเปลี่ยนแปลงเอกสารตามวันที่เอกสารที่เลือกได้ กรุณาติดต่อผู้ดูแลระบบ หรือผู้มีสิทธิ์กำหนดวันที่เอกสารได้")
      glbErrorLog.ShowUserError
      Exit Sub
   End If
   
   If m_HasModify Or (m_BalanceVerify.BALANCE_VERIFY_ID <= 0) Then
      glbErrorLog.LocalErrorMsg = MapText("กรุณาทำการบันทึกข้อมูลให้เรียบร้อยก่อน")
      glbErrorLog.ShowUserError
      Exit Sub
   End If

   Call InitMainButton(cmdMakeAdjust, MapText("WAIT"))
   cmdMakeAdjust.Enabled = False
   ...
   Call LoadLeftAmountLocation(InventoryBals2, -1, uctlDate.ShowDate)
   
   Set Ivd = New CInventoryDoc
   Ivd.ShowMode = SHOW_ADD
   Ivd.DOCUMENT_NO = "***A-TK_" & uctlDate.ShowDate & "_" & txtNo.Text
   Ivd.DOCUMENT_DATE = uctlDate.ShowDate
   Ivd.DOCUMENT_TYPE = ADJUST_DOCTYPE
   Ivd.DOCUMENT_DESC = "ตั้งยอดจาก " & txtNo.Text
   Ivd.CANCEL_FLAG = "N"
      
   Dim CR As CBalanceVerifyDeTail
   
   For Each CR In m_BalanceVerify.BalanceVerifyDetail
      Set m_LotItem = GetObject("CLotItem", InventoryBals2, Trim(CR.LOCATION_ID & "-" & CR.PART_ITEM_ID))
      
      If (Round(m_LotItem.SUM_AMOUNT, 2) <> (CR.PART_ITEM_AMOUNT)) Then
         ...
         If m_LotItem.SUM_AMOUNT > Val(CR.PART_ITEM_AMOUNT) Then 'ตอนนี้เป็น + เลยต้องการเบิกออก
            TempLotItem.TX_AMOUNT = m_LotItem.SUM_AMOUNT - CR.PART_ITEM_AMOUNT
            TempLotItem.MULTIPLIER = -1
            TempLotItem.TX_TYPE = "E"
         Else
            TempLotItem.TX_AMOUNT = CR.PART_ITEM_AMOUNT - m_LotItem.SUM_AMOUNT
            TempLotItem.MULTIPLIER = 1
   ...
   If Ivd.ImportExportItems.Count > 0 Then
      If Not glbDaily.AddEditInventoryDoc(Ivd, IsOK, True, glbErrorLog) Then
         glbErrorLog.ShowErrorLog (LOG_FILE_MSGBOX)
         Exit Sub
      End If
   End If
   ...
   Call InitMainButton(cmdMakeAdjust, MapText("ปรับSTOCK"))
   cmdMakeAdjust.Enabled = True
End Sub
...
Private Sub InitFormLayout()
   ...
   cmdMakeAdjust.Picture = LoadPicture(glbParameterObj.NormalButton1)
...
Dim RealIndex As Long
   ...
   If TabStrip1.SelectedItem.Index = 1 Then
      ...
      Values(5) = FormatNumber(CR.PART_ITEM_AMOUNT)
